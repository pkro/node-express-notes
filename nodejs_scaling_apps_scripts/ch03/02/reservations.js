...
const app = express()
    ...
    .post('/reserveTickets', (req, res) => {
        ...
        const show = shows.find(s => s._id === req.body.showID)
        if (!show) {
            res.status(500)
        ...
        const list = reservations[req.body.showID]
        const reservation = { name: req.body.name, guests: req.body.count }
        if (!list) {
...
app.listen(3002, () => console.log(`reservations running on port 3002`))
